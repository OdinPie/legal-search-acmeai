...
const maxTags = 5;
const SUGGESTIONS = ["Corporate", "Criminal", "Civil", "Finance", "Labor"];
...
export default function TagBar({ value = [], onChange }) {
  ...
  const addTag = (raw) => {
    ...
    if (has || value.length >= MAX_TAGS) return;
    onChange([...value, tag]);
    setInput("");
  };
    ...
    <div className="space-y-2">
      ...
      >
        ...
        <input
          ...
          disabled={atLimit}
          placeholder={atLimit ? "Max 5 tags reached" : "Add tag…"}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <span className="ml-auto text-xs text-gray-500">{value.length}/{MAX_TAGS}</span>
      </div>
    </div>
  );
}
