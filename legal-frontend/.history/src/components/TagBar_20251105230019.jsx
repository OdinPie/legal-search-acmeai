...
export default function TagBar({ value = [], onChange }) {
  ...
  return (
    <div className="my-2">
      <div
        className="flex items-center gap-2 overflow-auto overflo no-scrollbar h-8 w-full rounded-xl border border-gray-200 bg-white px-3"
        role="listbox"
      >
        {value.map(tag => (
          <span
            key={tag}
            className="inline-flex items-center gap-1 rounded-full bg-blue-50 text-blue-800 border border-blue-200 py-1 text-sm"
          >
            {tag}
            <button
